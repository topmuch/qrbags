...
const CATEGORY_LABELS: Record<string, { label: string; color: string }> = {
  actualites: { label: 'Actualités', color: 'bg-violet-100 text-violet-700 dark:bg-violet-900/30 dark:text-violet-400' },
  conseils: { label: 'Conseils', color: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-violet-500' },
  hajj: { label: 'Hajj 2026', color: 'bg-amber-100 text-amber-700 dark:bg-violet-900/30 dark:text-violet-500' },
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="mb-8">
        <Link
          ...
          className="inline-flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 hover:text-[#8b17c9] dark:hover:text-[#8b17c9] transition-colors mb-4"
        >
        ...
        <div className="flex items-center gap-3 mt-4">
          <div className="w-12 h-12 rounded-2xl bg-[#8b17c9]/20 flex items-center justify-center">
            <span className="text-2xl">📰</span>
          </div>
          <div>
            <h1 className="text-2xl font-bold text-slate-800 dark:text-white">Blog QRBags</h1>
      ...
      <div className="flex flex-wrap gap-2 mb-8">
        <Button
          ...
          className={`rounded-xl ${selectedCategory === 'all' 
            ? 'bg-[#8b17c9] hover:bg-[#8b17c9]/90 text-white' 
          ...
          {posts.map((post) => {
            ...
            return (
              <Link key={post.id} href={`/agence/blog/${post.slug}`}>
                <Card className="bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700 shadow-sm rounded-2xl hover:shadow-md hover:border-[#8b17c9]/30 transition-all cursor-pointer group">
                  <CardContent className="p-0">
                    <div className="flex flex-col sm:flex-row">
                      ...
                      <div className="flex-1 p-5">
                        ...
                        {/* Title */}
                        <h2 className="text-lg font-semibold text-slate-800 dark:text-white mt-2 group-hover:text-[#8b17c9] transition-colors">
                          {post.title}
                        </h2>
                        ...
                        {/* Read More */}
                        <div className="flex items-center gap-1 text-[#8b17c9] text-sm mt-3 font-medium">
                          Lire l'article
