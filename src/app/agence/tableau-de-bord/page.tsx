  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800 hover:shadow-lg hover:shadow-slate-200/50 dark:hover:shadow-slate-900/50 transition-all duration-300 group">
      <div className="flex items-start justify-between">
        ...
        {trend && (
          <div className={`flex items-center gap-1 text-sm font-medium ${trend.isUp ? 'text-violet-600' : 'text-rose-500'}`}>
  ...
  const getContextualSuggestions = (): { icon: string; title: string; text: string; color: string }[] => {
    ...
    if (stats.pending > 5) {
      suggestions.push({
        ...
        color: 'bg-amber-50 dark:bg-violet-600/10 border-amber-200 dark:border-amber-800 text-amber-700 dark:text-violet-500'
      });
    }
    ...
    if (stats.scanned > 0) {
      suggestions.push({
        ...
        color: 'bg-violet-50 dark:bg-violet-500/10 border-violet-200 dark:border-violet-800 text-violet-700 dark:text-violet-400'
      });
    }
    ...
    if (stats.total > 0 && stats.pending === 0 && stats.lost === 0) {
      suggestions.push({
        ...
        color: 'bg-emerald-50 dark:bg-violet-600/10 border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-violet-500'
      });
    }
    ...
    <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-200 dark:border-slate-800">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-violet-500 to-amber-600 flex items-center justify-center">
      ...
      {suggestion && !loading && (
        <div className="p-4 bg-gradient-to-r from-amber-50 to-violet-50 dark:from-violet-600/10 dark:to-violet-500/10 border border-amber-200 dark:border-amber-800 rounded-xl">
          <div className="flex items-start gap-3">
    ...
    <div className="relative rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
      {ad.imageUrl ? (
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="block p-6 bg-gradient-to-r from-amber-50 to-violet-50 dark:from-violet-600/10 dark:to-violet-500/10 text-center"
        >
...
export default function AgencyDashboardPage() {
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig: Record<string, { label: string; className: string }> = {
      ...
      pending_activation: { label: 'En attente', className: 'bg-amber-100 dark:bg-violet-600/10 text-amber-700 dark:text-violet-500' },
      active: { label: 'Actif', className: 'bg-emerald-100 dark:bg-violet-600/10 text-emerald-700 dark:text-violet-500' },
      scanned: { label: 'Scanné', className: 'bg-violet-100 dark:bg-violet-500/10 text-violet-700 dark:text-violet-400' },
      ...
      EN_ATTENTE: { label: 'En attente', className: 'bg-amber-100 dark:bg-violet-600/10 text-amber-700 dark:text-violet-500' },
      ACTIF: { label: 'Actif', className: 'bg-emerald-100 dark:bg-violet-600/10 text-emerald-700 dark:text-violet-500' },
      SCANNÉ: { label: 'Scanné', className: 'bg-violet-100 dark:bg-violet-500/10 text-violet-700 dark:text-violet-400' },
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-slate-800 dark:text-white">
          Bienvenue, <span className="text-violet-600">{agencyName}</span>
        </h1>
        ...
        <>
          ...
          {activatedBaggages.length > 0 && (
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden mb-6">
              <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-800 bg-emerald-50/50 dark:bg-violet-600/5">
                <div className="flex items-center gap-2">
                        ...
                        <td className="px-6 py-4">
                          <div>
                            ...
                            {baggage.travelerFirstName || baggage.travelerLastName ? (
                              ...
                              <div className="flex items-center gap-2">
                                <span className="px-2 py-1 bg-amber-100 dark:bg-violet-600/20 text-amber-600 dark:text-violet-500 rounded-full text-xs font-medium">
                                  Non assigné
                                </span>
                                <button
                                  onClick={() => {
                                  ...
                                  className="text-xs text-[#8b17c9] hover:text-[#ff9f00]"
                                >
                                  Attribuer
                                </button>
                              </div>
                            ...
                            <div>
                              ...
                              {baggage.founderPhone && (
                                <a 
                                  ...
                                  target="_blank"
                                  rel="noopener noreferrer"
                                  className="text-violet-600 hover:text-violet-700 text-xs flex items-center gap-1 mt-0.5"
                                >
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {isLost(baggage.status) && (
                              <button
                                ...
                                className="p-2 rounded-lg hover:bg-emerald-50 dark:hover:bg-violet-600/10 transition-colors group"
                                title="Marquer comme retrouvé"
                              >
                                <CheckCircle className="w-4 h-4 text-slate-400 group-hover:text-violet-600" />
                              </button>
                            )}
                            <button
                              onClick={() => {
          ...
          {pendingBaggages.length > 0 && (
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-amber-200 dark:border-amber-800 overflow-hidden">
              <div className="px-6 py-4 border-b border-amber-200 dark:border-amber-800 bg-amber-50/50 dark:bg-violet-600/5">
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full bg-violet-600" />
                  <h2 className="text-sm font-semibold text-slate-800 dark:text-white">
                  ...
                  <tbody>
                    {pendingBaggages.map((baggage) => (
                      ...
                      >
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <div className="w-8 h-8 rounded-lg bg-amber-100 dark:bg-violet-600/10 flex items-center justify-center">
                              <QrCode className="w-4 h-4 text-violet-600" />
                            </div>
                            <span className="text-slate-800 dark:text-white font-mono font-medium">
                              {baggage.reference}
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="px-3 py-1.5 bg-[#8b17c9] hover:bg-[#ff9f00] text-white rounded-lg text-xs font-medium transition-colors"
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-md w-full shadow-xl border border-slate-200 dark:border-slate-800">
            <div className="flex items-center justify-between p-6 border-b border-slate-200 dark:border-slate-800">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-amber-100 dark:bg-violet-600/10 flex items-center justify-center">
            ...
            {commandSuccess ? (
              <div className="p-8 text-center">
                <div className="w-16 h-16 rounded-full bg-emerald-100 dark:bg-violet-600/10 flex items-center justify-center mx-auto mb-4">
              ...
              <div className="p-6 space-y-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full p-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-700 dark:text-slate-200 focus:ring-2 focus:ring-violet-600/20 focus:border-violet-600"
                  >
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full p-3 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-700 dark:text-slate-200 placeholder-slate-400 focus:ring-2 focus:ring-violet-600/20 focus:border-violet-600 resize-none"
      ...
      {showDetailModal && selectedBaggage && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-md w-full max-h-[90vh] overflow-y-auto shadow-xl border border-slate-200 dark:border-slate-800">
            ...
            <div className="p-6 space-y-4">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 bg-amber-100 dark:bg-violet-600/10 rounded-xl flex items-center justify-center">
              ...
              <div>
                ...
                {selectedBaggage.whatsappOwner ? (
                  ...
                  <span className="text-amber-600 dark:text-violet-500 text-sm">Non renseigné</span>
                )}
              </div>

              {/* Edit Form for unassigned baggages */}
              {(!selectedBaggage.travelerFirstName && !selectedBaggage.travelerLastName) && (
                <div className="p-4 bg-amber-50 dark:bg-violet-600/10 border border-amber-200 dark:border-amber-800 rounded-xl">
                  <h4 className="text-amber-700 dark:text-violet-500 font-medium mb-3">Attribuer ce bagage</h4>
                  <div className="space-y-3">
                    ...
                    <button
                      ...
                      className="w-full py-2 bg-[#8b17c9] hover:bg-[#ff9f00] text-white rounded-lg text-sm font-medium transition-colors"
                    >
                      Enregistrer
                    </button>
                  </div>
              ...
              {selectedBaggage.founderName && (
                <div className="p-4 bg-emerald-50 dark:bg-violet-600/10 border border-emerald-200 dark:border-emerald-800 rounded-xl">
                  <div className="flex items-center gap-2 mb-3">
                    <CheckCircle className="w-5 h-5 text-violet-600" />
                    <p className="text-emerald-700 dark:text-violet-500 font-medium">Trouvé par</p>
                  </div>
